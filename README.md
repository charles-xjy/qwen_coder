...
- 每条记忆注入时携带精确相对时间（"5 分钟前" / "3 小时前" / "2 天前"），超过 1 天还额外附新鲜度警告

**每轮对话涉及两次独立 LLM 调用：**

| 调用 | 时机 | 方式 | 说明 |
|------|------|------|------|
| sideQuery（检索） | 主 LLM 调用前 | 串行 await | 将带时间戳的 MEMORY.md 索引发给 LLM，选出最相关的 ≤5 条（含可能冲突的新旧两条），包装成一条独立消息 **append 进消息历史** |
| auto_save_memory（写入） | 主 LLM 最终回复后 | `asyncio.create_task` fire-and-forget | 分析本轮对话，判断是否有值得长期保存的内容，有则直接写文件；写路径不做冲突解决，冲突交由读时裁决 |
...
| 冲突对全选 | manifest 带相对时间；提示词明确要求"描述相近/可能冲突的新旧两条全部选出"，避免只捞到旧的、漏掉新的 |

**读时冲突裁决机制**

本系统采用**读时解决**策略：写路径保持轻量的 fire-and-forget，冲突在每次召回时由主 LLM 按时间戳裁决。

完整流程（以"先说喜欢 Python，几分钟后改口喜欢 Java"为例）：

```
① 写入阶段（两次独立 fire-and-forget，互不感知）：
   t=0    用户："我喜欢 Python" → auto_save → 写 user_编程语言偏好.md (mtime=t0)
   t=5min 用户："我喜欢 Java"  → auto_save → 写 user_java偏好.md     (mtime=t1)
   磁盘：两条并存，内容矛盾
...
② 检索阶段（下轮 sideQuery）：
   ...
   sideQuery 提示词要求"冲突的新旧两条全部选出" → 两条都被选中

③ 注入阶段（发给主 LLM 的 system prompt 记忆区块头部）：
...
④ 主 LLM 裁决：
   看到两条时间差（7 分 vs 2 分）→ 判断为冲突 → 采信"Java（较新）"
   若两条不矛盾（如"Python 做数据分析" + "Java 做后端"）→ 两者并存

⑤ AutoDream 收尾（≥24h + ≥5 会话触发）：
   物理删除矛盾的旧条目，磁盘最终干净
```

**为什么用读时解决而非写时解决**：写路径是 fire-and-forget，加入"先检索再写"会引入阻塞延迟，且每轮都多一次 LLM 调用；读时裁决利用已有的 sideQuery 调用顺带处理，zero 额外成本。代价是冲突记忆在 AutoDream 前会物理并存，但即时对话不受影响——主 LLM 每轮都能看到正确的新值。

**AutoDream：定期记忆整合（对齐 cc-haha autoDream）**

积累足够多的会话后，在会话结束时自动触发一次深度整合：合并重复条目、删除过时/矛盾记忆、更新陈旧内容。这是冲突解决的**物理清理层**——读时裁决保证当下对话正确，AutoDream 负责让磁盘最终干净。
